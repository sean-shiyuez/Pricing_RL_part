...
class RevenueEvalCallback(BaseCallback):
    def __init__(self, env, eval_freq=100, verbose=1, tensorboard_log_dir="./dqn_tensorboard/"):
...
def train(env, total_timesteps=10000):
    ...
    model = DQN("MlpPolicy", env, learning_rate=0.0001, verbose=1, tensorboard_log=tensorboard_log_dir)
    callback = RevenueEvalCallback(env, eval_freq=100, tensorboard_log_dir=tensorboard_log_dir)
    model.learn(total_timesteps=total_timesteps, callback=callback)
    model.save("dqn_charging_station")
...
def test_model(env, model_path="dqn_charging_station"):
...
def main():
    ...
    if mode == 'train':
        ...
        test_model(env, model_path="dqn_charging_station")
    else:

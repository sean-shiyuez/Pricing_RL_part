...
class ChargingStationEnv(gym.Env):
    def __init__(self):
        ...
        # Define action space: 10 discrete price options (0.1 to 1.0 USD/KWh) for each of our stations
        self.n_prices = 10
        self.n_our_stations = sum(self.Station_specs['competitor'] == False)
        self.action_space = spaces.MultiDiscrete([self.n_prices] * self.n_our_stations)
        self.prices = np.linspace(0.1, 1.0, self.n_prices)

        # Define observation space
        self.n_regions = self.n_our_stations  # Number of our charging stations
        self.total_time_slots = 3  # Number of occupancy time slots for each charging station
        # State includes: occupancy for each station, previous prices for each station, competitor price, current total revenue
        self.state_size = (self.n_regions * self.total_time_slots) + self.n_regions + 2
    ...
    def reset(self):
        ...
        # Initial previous period prices (one for each station)
        p_previous = np.zeros(self.n_our_stations)
    ...
    def step(self, action):
        ...
        # Convert action indices to actual prices for each station
        our_prices = self.prices[action]
        self.transition(self.current_step, our_prices)

        our_stations = self.Station_specs[self.Station_specs['competitor'] == False]['id'].tolist()
        occupancy_our = self.occupancy_tensor.loc[our_stations].values.flatten()
        p_previous = our_prices
        p_com = self.copetitors_prices.mean()
        R_now = self.total_revenue_tensor.sum().sum()
        self.state = np.concatenate([occupancy_our, p_previous, [p_com, R_now]])

        # Calculate reward based on all stations
        ki = 0.15
        sum_o = occupancy_our.sum()
        Ri = ki * sum_o * np.mean(our_prices)  # Use average price for reward calculation
        kt = 0.5
    ...
    def transition(self, hour, our_prices):
        # Update prices for each station
        for i, station_id in enumerate(self.our_price_tensor.index):
            self.our_price_tensor.loc[station_id, hour] = our_prices[i]
        ...
        for sta_id in self.Station_specs['id']:
            if self.Station_specs.loc[self.Station_specs['id'] == sta_id, 'competitor'].values[0]:
                ...
                # Get the corresponding price for our station
                station_idx = self.our_price_tensor.index.get_loc(sta_id)
                price = our_prices[station_idx]
        ...
        # Update revenue for each station
        for i, station_id in enumerate(self.our_price_tensor.index):
            station_occupancy = new_occupancy_tensor.loc[station_id]
            self.our_revenue_tensor.loc[station_id, hour] = (station_occupancy * our_prices[i]).sum()
        ...
        self.our_previous_prices = pd.DataFrame(
            np.zeros((self.n_our_stations, 1)),
            index=self.our_price_tensor.index
        )
        if hour > 0:
